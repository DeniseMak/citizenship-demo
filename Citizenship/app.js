...
const USE_LUIS_CLIENT = 0;
const USE_LUIS_RECOGNIZE = 0;
const USE_MS_SENTIMENT = 1;
...
bot.dialog('AskQuestionDialog', [
    ...
    function (session, results) {
        ...
        var curr_q_score = judgeAnswer(qId, lastUtterance, session);

        // Get sentiment analysis and luis score
        var ms_sentiment_score = -1;
        var luis_score = -1;

        // Call LUIS
    builder.LuisRecognizer.recognize(lastUtterance,
        "https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/2a7e6e0e-8199-4350-a6b7-9b2fe9ffc18a?subscription-key=9823b75a8c9045f9bce7fee87a5e1fbc&verbose=true",
        function (err, intents, entities) {
            ...
            if (typeof (intents) != undefined) {
                console.log(intents)
                if (intents[0].intent === 'CorrectAnswer') { luis_score = 1; return 1; }
        ...
        sentimentAnalysis(results.response).then(analyticsResponse => {
            console.log(`Analyzing sentiment of: "${results.response}" in dialog.`);
            ms_sentiment_score = Math.round(analyticsResponse["documents"][0]["score"] * 100) / 100;
            console.log(`The sentiment score of that utterance is ${ms_sentiment_score}`);

            /*****
             *  Try moving everything from here up to end of dialog into .then clause of promise.
             * 
             */
            // update total score
            session.privateConversationData.test.score += curr_q_score;

            console.log('curr_q_score = %d, and ms_sentiment_score = %d, and luis_score = %d', curr_q_score, ms_sentiment_score, luis_score);
            if (curr_q_score > .6) {
                goodOKBad = 'Good answer';
            } else if (ms_sentiment_score > .4 || curr_q_score > .4) {
                goodOKBad = 'That answer was not exactly correct but close enough.';
            } else {
 ...
 * 
 */
function judgeAnswer(qId, utterance, session) {
    ...
    switch (qId) {
        ...
        case (19): // What was one important thing that Abraham Lincoln did?
            ...
            if (typeof(utterance) !== undefined && utterance !== null) {
                ...
                // if using LUISClient do predict  
if (USE_LUIS_CLIENT) {
...
if (USE_LUIS_RECOGNIZE) {
    /***************
     *  Callback is (err: Error, intents?: IIntent[], entities?: IEntity[]): void
     */
    builder.LuisRecognizer.recognize(utterance,
        "https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/2a7e6e0e-8199-4350-a6b7-9b2fe9ffc18a?subscription-key=9823b75a8c9045f9bce7fee87a5e1fbc&verbose=true",
        function (err, intents, entities) {
            if (err) {
                console.error(err);
            ...
            if (typeof (intents) != undefined) {
                console.log(intents)
                if (intents[0].intent === 'CorrectAnswer') { score = 1; return 1; }
...
/****
* Call the Microsoft sentiment analysis API from within this function
* i.e., consume the promise
*/

var MsScore;

var PrintScoresPromise = sentimentAnalysis(utterance).then(analyticsResponse => {
    // First, get the MsScore
    console.log('starting MS text analysis:');
    MsScore = Math.round(analyticsResponse["documents"][0]["score"] * 100) / 100;

}).then(
    onfulfilled => {
        console.log(`The MSscore of that utterance is ${MsScore}`);
        if (MsScore > 0.7) {
            console.log("Wow that's positive!");
        } else if (MsScore > 0.3) {
            console.log("Wow %d that's neutral-ish...", MsScore);
        } else {
            console.log("Wow that is not positive.");
        }

        // Next, get the regex score
        console.log('judgeAnswer RegEx matching: utterance = %s', utterance);
        ...
        if (match !== null) {
            ...
            RegexScore = 0;
            console.log('judgeAnswer Regex: Didn\'t match utterance = %s', utterance);
        }

        if (MsScore > 0) {
            console.log('returning MsScore score = %d', MsScore)
            return MsScore;
        } else {
            console.log('returning regex score = %d', RegexScore)
            return RegexScore;
        }
    }
)
        ...
        // Next, get the regex score
        console.log('judgeAnswer RegEx matching (OUTSIDE OF .then): utterance = %s', utterance);
        var match = utterance.match(/.*(save|preserve).*(union|country|nation).*|.*emancipation proclamation.*|(free|emancipate|liberate).*slave*.*|(end|abolish).*slavery.*/i);
        var RegexScore;
        if (match !== null) {
            RegexScore = 1;
            ...
            RegexScore = 0;
            console.log('judgeAnswer Regex (OUTSIDE OF .then): Didn\'t match utterance = %s', utterance);           
        }
        
        return RegexScore;
            } else {
                console.log("null or undefined utterance for qId=19");
                return RegexScore;
            }
...
/* ********************************************** */
/* This function calls the Microsoft Sentiment Analysis API */
/* ********************************************** */
let sentimentAnalysis = function (string) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        'https://westus.api.cognitive.microsoft.com/text/analytics/v2.0/sentiment/', 
            {
                "headers": {
                    "Ocp-Apim-Subscription-Key": "f95829d0f74d46a6b14af374ffc47c45",// process.env.TEXT_ANALYTICS_KEY,
...
// Wrap JudgeAnswer in a Promise
var promiseJudgeAnswer = function (qId, utterance, session) {

    return new Promise(function(resolve, reject) {
        try {
            return resolve(judgeAnswer(qId, utterance, session));
        } catch (ex) {
            reject(ex);
        }
    })
